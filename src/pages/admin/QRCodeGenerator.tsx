...
export default function QRCodeGenerator({ store }) {
  ...
  return <section className="max-w-lg space-y-5">
    ...
    <div data-qr-surface className="rounded-2xl border bg-white p-8 text-center" ref={ref}><h3 className="mb-4 text-xl font-bold" style={{color:'#111'}}>{store.name}</h3><QRCodeSVG className="mx-auto max-w-full h-auto" value={url} size={256} marginSize={4} bgColor="#ffffff" fgColor="#000000" /><p className="mt-3 break-all" dir="ltr" style={{color:'#111'}}>{url}</p></div>
